use amethyst::{
    core::ecs::{
        Component, DenseVecStorage, DispatcherBuilder, Join, WriteStorage, SystemData, World
    },
    prelude::*,
    renderer::{
...
impl<B: Backend> RenderGroup<B, World> for DrawCustom<B> {
    fn prepare(
        ...
        let (triangles,) = <(WriteStorage<'_, Triangle>,)>::fetch(world);
        // Get our scale value
        let mut scale = world.write_resource::<CustomUniformArgs>();
        scale.iTime = scale.iTime % 3.14  + 0.01;
...
impl<B: Backend> RenderPlugin<B> for RenderCustom {
    fn on_build<'a, 'b>(
        ...
        world.insert(CustomUniformArgs { scale: 1.0, iTime : 0.0});
        Ok(())
    }
